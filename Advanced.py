...
# ---------------- GRAD-CAM (KERAS 3 SAFE - LAYER BY LAYER) ----------------
def _safe_call(layer, x, training=False):
    """Call layer with training if supported, else call without."""
    try:
...
def find_last_conv_in_backbone(backbone: tf.keras.Model):
    """Find last Conv-like layer inside backbone (EfficientNet)."""
...
def compute_gradcam(model: tf.keras.Model, img_tensor: tf.Tensor, class_index: int):
    """
    Keras 3-safe Grad-CAM for a model like:
    input -> data_augmentation -> efficientnetb0(backbone) -> GAP -> Dense
    """
    # Backbone must exist
    try:
        backbone = model.get_layer("efficientnetb0")
    except Exception:
        raise ValueError("Model has no layer named 'efficientnetb0'.")

    conv_name = find_last_conv_in_backbone(backbone)
    if conv_name is None:
        raise ValueError("No conv layer found inside efficientnetb0 backbone.")

    conv_layer = backbone.get_layer(conv_name)

    # Backbone CAM model: from backbone.input -> [last_conv_output, backbone_output]
    backbone_cam = tf.keras.Model(
        inputs=backbone.input,
        outputs=[conv_layer.output, backbone.output]
    )

    # Run full model layer-by-layer so tensors are connected correctly
    with tf.GradientTape() as tape:
        x = img_tensor
        conv_outputs = None

        # skip the InputLayer to avoid call issues
        for layer in model.layers:
            if isinstance(layer, tf.keras.layers.InputLayer):
                continue

            if layer.name == "efficientnetb0":
                conv_outputs, x = backbone_cam(x, training=False)  # x becomes backbone features
            else:
                x = _safe_call(layer, x, training=False)

        preds = x  # final output

        # Ensure preds is (batch, num_classes)
        preds_2d = tf.reshape(preds, [tf.shape(preds)[0], -1])
        if preds_2d.shape[-1] is not None:
            class_index = min(class_index, int(preds_2d.shape[-1]) - 1)

        loss = preds_2d[:, class_index]

    if conv_outputs is None:
        raise ValueError("Could not capture conv outputs from backbone (unexpected model structure).")

    grads = tape.gradient(loss, conv_outputs)
    if grads is None:
        raise ValueError(f"Gradients are None. Try a different conv layer inside efficientnetb0.")
    ...
    used_layer = f"efficientnetb0.{conv_name}"
    return heatmap.numpy(), used_layer


def overlay_gradcam(image: Image.Image, heatmap, alpha=0.4):
...
def load_models():
    ...
    try:
        ...
        st.warning("Custom CNN model not found (cnn_model (1).h5).")

    try:
        effnet = load_model("efficientnetb0_finetuned.keras", compile=False)
    except Exception:
        st.warning("EfficientNet model not found (efficientnetb0_finetuned.keras).")
...
def generate_pdf_report(
    ...
    if gradcam_image is not None:
        ...
        img_table.append([Paragraph("<b>Original</b>", styles["Normal"])])
        img_table.append([orig_img])
        col_widths = [7*cm]

    table = Table(img_table, colWidths=col_widths)
    ...
    for model_name, preds in top3_details.items():
        ...
        t = Table(table_data, colWidths=[2*cm, 7*cm, 4*cm])
...
if len(available_models) == 2:
    available_models.append("Compare Both")

if not available_models:
    st.error("No models loaded. Put your model files next to app.py then rerun.")
    st.stop()
...
def is_image_blurry(image, threshold=100.0):
    ...
    return lap_var < threshold, float(lap_var)
...
if image_input is not None:
    ...
    # EfficientNet batch (preprocess correctly)
    eff_arr = np.array(resized).astype(np.float32)
    eff_batch = np.expand_dims(eff_arr, 0)
    eff_batch = efficient_preprocess(eff_batch)  # IMPORTANT
    eff_tensor = tf.convert_to_tensor(eff_batch)
    ...
    # ---- Custom CNN
    if model_choice in ["Custom CNN", "Compare Both"] and cnn_model is not None:
        start = time.time()
        ...
        best_class_indices["Custom CNN"] = top_idx
        best_predictions["Custom CNN"] = CLASS_NAMES[top_idx]
        best_confidences["Custom CNN"] = float(preds[top_idx])

    # ---- EfficientNetB0
    if model_choice in ["EfficientNetB0", "Compare Both"] and effnet_model is not None:
        start = time.time()
        ...
        top_idx = int(np.argmax(preds))
        best_class_indices["EfficientNetB0"] = top_idx
        best_predictions["EfficientNetB0"] = CLASS_NAMES[top_idx]
        best_confidences["EfficientNetB0"] = float(preds[top_idx])
    ...
    if len(best_predictions) == 2:
        if best_predictions["Custom CNN"] == best_predictions["EfficientNetB0"]:
            ...
            final_conf = max(best_confidences["Custom CNN"], best_confidences["EfficientNetB0"])
        ...
        only_model = list(best_predictions.keys())[0]
        final_class = best_predictions[only_model]
        final_conf = best_confidences[only_model]
        agreement_text = "Single model result"
    ...
    with col3:
        ...
        for model_name, preds in top3_details.items():
            ...
            st.markdown(f"**{model_name}** ({agreement_text})")
            st.progress(float(best_cf))
    ...
    can_show_gradcam = (
        ...
        and ("EfficientNetB0" in best_class_indices)
        and (model_choice in ["EfficientNetB0", "Compare Both"])
        and (final_conf >= HIGH_CONF_THRESHOLD)
        and (agreement_text.startswith("🟢") or agreement_text == "Single model result")
    )
    ...
    if can_show_gradcam:
        ...
        effnet_class_idx = best_class_indices["EfficientNetB0"]

        try:
            heatmap, used_layer = compute_gradcam(effnet_model, eff_tensor, effnet_class_idx)
            gradcam_image = overlay_gradcam(image_input, heatmap)

            st.session_state["gradcam_image"] = gradcam_image

            st.caption(f"Grad-CAM layer used: {used_layer}")
            st.image(gradcam_image, caption="Grad-CAM: Model Focus Regions", use_column_width=True)
            st.info("🧠 Grad-CAM highlights regions that most influenced the model’s prediction.")
        except Exception as e:
            st.session_state["gradcam_image"] = None
            st.error(f"Grad-CAM failed: {e}")
    else:
        st.session_state["gradcam_image"] = None
        st.warning("Grad-CAM suppressed due to low confidence, disagreement, or unsupported input.")
